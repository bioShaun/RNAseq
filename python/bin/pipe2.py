...
mRNA_pipe_v2.py run_pipe \\
    ...
    --database {6} \\
    --database-version {7} \\
        '''.format(self.proj_name, self.proj_dir, self.clean_dir, self.sample_inf, self.species, self.worker_number, self.database, self.database_version)
        if self.kegg_bg:
            pipe_inf = '{0}    --kegg-bg {1} \\\n'.format(pipe_inf, self.kegg_bg)
